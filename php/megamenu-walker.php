    ...
    {     
        ...
        function start_lvl(&$output, $depth) 
        {
            ...
            if ($depth == 0) { //really, level-1 or level-2, because $depth is misleading here (see note above) 
                $output .= "\n{$tabs}<ul class=\"dropdown-menu mega-menu\"><div>\n"; 
            } else if($depth == 1){ 
                $output .= "\n{$tabs}<ul class=\"mega-item\">\n"; 
            } 
            return;
        } 
            ...
            if ($depth == 0) {
            	if($item->hasChildren) {
            		if ( $this->is_active($item) ){
            			$class_names = ' class="dropdown active"';		
            		}
					else {
		...
		function is_active($item)
		{
			...
			$item_slug = sanitize_title( $item->name );
			
			if( is_home() && $item_slug == 'home' ) {
				return true;
			}
			
			$current_name = get_post($wp_query->post->ID)->post_name;
			
			if( $current_name == $item_slug) {
				return true;
			}
